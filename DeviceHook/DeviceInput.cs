...
namespace ryu_s.DeviceHook
{
    ...
    {
        public enum MouseClickButtonType
        {
            Left,
            LeftDown,
            LeftUp,
        ...
        /// マウスのホイールの動きをシミュレート
        /// </summary>
        /// <param name="amount">動かす量。120の倍数を指定する。</param>
        /// <remarks>
        /// typeがVerticalの時、amountがマイナスだと左に、プラスだと右に動く。
        /// typeがHorizontalの時、amountがマイナスだと下に、プラスだと上に動く。
        /// </remarks>
        public static void MoveMouseWheel(MouseWheelType type, int amount)
        {
            var u = new InputUnion();
            u.mi.mouseData = amount;
            if (type == MouseWheelType.Horizontal)
                u.mi.dwFlags = MOUSEEVENTF.HWHEEL;
            else
        ...
        public static void MouseClick(MouseClickButtonType type)
        {
        ...
        public static void TestClick()
        {
            ...
            InputUnion union0 = new InputUnion();
            union0.mi.dx = 0;
            union0.mi.dy = 0;
            union0.mi.mouseData = 0;
            union0.mi.dwFlags = MOUSEEVENTF.LEFTDOWN;
            list.Add(new INPUT() { type = INPUT_TYPE.INPUT_MOUSE, U = union0 });

            InputUnion union1 = new InputUnion();
            union1.mi.dx = 0;
            union1.mi.dy = 0;
            union1.mi.mouseData = 0;
            union1.mi.dwFlags = MOUSEEVENTF.LEFTUP;
            list.Add(new INPUT() { type = INPUT_TYPE.INPUT_MOUSE, U = union1 });
        ...
        private struct MOUSEINPUT
        {
            ...
            /// イベントを起こすタイムスタンプ。ミリ秒。
            /// </summary>
            /// <remarks>0の場合はシステムがその時の時間を設定。つまり即時実行</remarks>
            internal uint time;
            /// <summary>
            /// GetMessageExtraInfoで取得できる値を指定する。
            /// </summary>
        ...
        private static extern uint _sendInput(uint nInputs,
           [MarshalAs(UnmanagedType.LPArray), In] INPUT[] pInputs,
           int cbSize);
        private static uint SendInput(IEnumerable<INPUT> Inputs)
        {
